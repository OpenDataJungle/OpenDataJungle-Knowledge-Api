...
import com.laulem.vectopath.knowledge.api.infra.dto.GroupUserResponse;
import com.laulem.vectopath.knowledge.api.infra.dto.PaginatedResponse;
import com.laulem.vectopath.knowledge.api.infra.dto.UserResponse;
import com.laulem.vectopath.knowledge.api.infra.properties.VectoPathReferentialApiProperties;
...
public class ReferentialRepository {
    ...
    private final VectoPathReferentialApiProperties vectoPathReferentialApiProperties;
    private final AuthenticationUseCase authenticationUseCase;

    public ReferentialRepository(RestClient.Builder restClientBuilder, VectoPathReferentialApiProperties vectoPathReferentialApiProperties, AuthenticationUseCase authenticationUseCase) {
        this.restClient = restClientBuilder.baseUrl(vectoPathReferentialApiProperties.getBaseUrl()).build();
        this.vectoPathReferentialApiProperties = vectoPathReferentialApiProperties;
    ...
    public boolean hasGroupWriteAccess(final List<UUID> groupIds, final String currentUser) {
        ...
        try {
            ...
            PaginatedResponse<GroupUserResponse> response = restClient.get()
                    .uri(this.vectoPathReferentialApiProperties.getUserGroupsByUserId(), userId)
    ...
    private UUID getUserId(final String currentUser) {
        try {
            ...
            UserResponse userResponse = restClient.get()
                    .uri(this.vectoPathReferentialApiProperties.getUserByUsername(), currentUser)
                    .header(AUTHORIZATION, BEARER + bearerToken)
                    .retrieve()
