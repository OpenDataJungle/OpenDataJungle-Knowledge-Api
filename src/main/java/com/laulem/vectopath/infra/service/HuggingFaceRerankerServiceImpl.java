...
public class HuggingFaceRerankerServiceImpl implements RerankerService {
    private static final Logger logger = LoggerFactory.getLogger(HuggingFaceRerankerServiceImpl.class);

    private static final ParameterizedTypeReference<List<RerankResult>> RESULT_TYPE = new ParameterizedTypeReference<>() {
    };

    private final RestClient restClient;

    public HuggingFaceRerankerServiceImpl(RestClient restClient) {
    ...
    public List<PartialResource> rerank(String query, List<PartialResource> candidates, int limit) {
        ...
        try {
            List<RerankResult> results = restClient
                    .post()
                    .uri("/rerank")
