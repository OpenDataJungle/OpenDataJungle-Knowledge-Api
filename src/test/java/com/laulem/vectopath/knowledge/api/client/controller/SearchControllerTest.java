...
class SearchControllerTest {
    ...
    void searchSemantic_shouldDelegateToService_andMapResults() {
        ...
        SearchRequest request = new SearchRequest("what is vectopath", 5, 0.7, resourceIds);
        PartialResource partialResource = new PartialResource();
        partialResource.setResourceId(UUID.randomUUID());
        partialResource.setContent("vectopath is a search platform");
        when(vectorizedResourceService.searchSimilar("what is vectopath", 5, 0.7, resourceIds))
        ...
        verify(vectorizedResourceService).searchSimilar("what is vectopath", 5, 0.7, resourceIds);
    }
